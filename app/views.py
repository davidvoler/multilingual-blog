...
def form(request):
    ...
    return render_to_response('app/form.html', {}, RequestContext(request))
"""
def form(request):
    name = setting('SOCIAL_AUTH_PARTIAL_PIPELINE_KEY', 'partial_pipeline')
    email_r=request.REQUEST.get('email','')
    try:
        email_s=request.session['email']
    except:
        email_s=False
    if email_r:
        username=email_r
...
def form2(request):
    if request.method == 'POST' and request.POST.get('first_name'):
        ...
        backend = request.session[name]['backend']
        return redirect('socialauth_complete', backend=backend)
    return render_to_response('app/form2.html', {}, RequestContext(request))
